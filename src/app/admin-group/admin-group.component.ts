...
export class AdminGroupComponent implements OnInit {
	...
	//Create group object and push superAdmins and groupAdmins
	createGroup() {
		for (var index in this.users) {
			if (
				this.users[index].role === "SuperAdmin" ||
				this.users[index].role === "GroupAdmin"
			) {
		...
		this.httpClient
			.post(
				`${this.url}/api/insertGroup`,
				JSON.stringify(groupObject),
				httpOptions,
			)
			.subscribe((result: any) => {
				if (result === true) {
	...
	deleteGroup(group: any) {
		this.httpClient
			.post(`${this.url}/api/deleteGroup`, JSON.stringify(group), httpOptions)
			.subscribe((data: any) => {
				if (data === true) {
	...
	//Takes user and group names and deletes the user from it
	...
	//Checks if the param (member) is an admin, otherwise returns true
	...
	addMembers(user: any, group: any) {
		...
		} else if (this.memberRoleCheck(user) === false) {
			//If admin
			alert("This user is already in the group!");
		} else {
