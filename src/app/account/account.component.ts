...
export class AccountComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.userRole == "SuperAdmin" || this.userRole == "GroupAdmin") {
      //get groups
      this.httpClient.get(this.BACKEND_URL + "/api/getGroups").subscribe((result: any) => {
      ...
      //get users
      this.httpClient.get(this.BACKEND_URL + "/api/getUsers").subscribe((result: any) => {
        for (let i = 0; i < result.length; i++) {
          this.users.push(result[i]);
        }
      });
      //get channels, delete any lingering channels with no groups
      this.httpClient.get(this.BACKEND_URL + "/api/getChannels").subscribe((result: any) => {
        for (let i=0; i < result.length; i++) {
          ...
          if (this.groupTitles.includes(result[i].groupName)) {
            ...
            this.httpClient.post(this.BACKEND_URL + "/api/deleteChannel", JSON.stringify(result[i]), httpOptions).subscribe((data: any) => {
              if (data) {
  ...
  deleteUser(user: any) {
    this.httpClient.post(this.BACKEND_URL + "/api/deleteUser", JSON.stringify(user), httpOptions).subscribe((data: any) => {
      if (data == true) {
  ...
  deleteGroup(group: any) {
    this.httpClient.post(this.BACKEND_URL + "/api/deleteGroup", JSON.stringify(group), httpOptions).subscribe((data: any) => {
      if (data == true) {
