...
export class LoginComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.user != null) {
      this.httpClient.post(this.url + "/api/auth", JSON.stringify(this.user), httpOptions).subscribe((data: any) => {
        if (data.valid) {
  ...
  validate(username: any, password: any) {
    ...
    if (username.length > 0) { //Not empty
      for (let i=0; i < username.length; i++) { //No spaces
